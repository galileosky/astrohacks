/* Adapted for Arduino from: ellipse.c
...
int n, float *p, float *e, int ip[PARS], int nfree)
{
  ...
  cosp = cos(F0 * p[4]); 	/* cosine of p.a. */
  sinp = sin(F0 * p[4]); 	/* sine of p.a. */
  cosi = cos(F0 * p[1]); 	/* cosine of inclination */
  sini = sin(F0 * p[1]); 	/* sine of inclination */
  for (i = 0; i < n; i++) {
    double cost, sint, u;
...
float *x, float *y, int n, int ip[PARS], int nfree)
{
  ...
  if (invmat(s1, nfree))
    return (-4); 		/* error inverting matrix */
  ...
  (*q) = 0.0;
  cosp = cos(F0 * e[4]); 	/* cosine of p.a. */
  sinp = sin(F0 * e[4]); 	/* sine of p.a. */
  cosi = cos(F0 * e[1]); 	/* cosine of inclination */
  for (i = 0; i < n; i++) {
    double cost, sint, u;
    ...
    sint =
      (-(x[i] - e[2]) * cosp - (y[i] - e[3]) * sinp) / e[0] / cosi;
    u = 1.0 - cost * cost - sint * sint; 	/* difference with model */
...
{ 				/* ellipse parameters */
   ...
   */
  {
    ...
    if (pa < 0.0)
      pa += 180.0; 	/* position angle in range 0.....180 */
    p[0] = ab; 		/* radius */
